...
{
   { COMM_PORT_DOS_DRIVER, &dos_comm_port_func_table, TRUE },
   { 0,                    NULL,                     0 }
};
...
#ifdef DEBUGMODE
inline void DEBUG_MARKER(fifo_queue *q, unsigned char x) {
queue_put_(q, &x);
}
#else
...
int dos_comm_port_install_handler(comm_port *port)
{
   ...
   if (irq_top_com_port[port->nIRQ] == NULL) irq_top_com_port[port->nIRQ] = port;
   else irq_bot_com_port[port->nIRQ]->next_port = port;
    ...
    */
   dz_disable_interrupts(); /* Stop interrupts for a few critical operations */
   if (inportb(port->MSR) & CTS_IN_MSR) port->cts = CTS_ON;
...
void dos_comm_port_uninstall_handler(comm_port *port)
{
   ...
   while (queue_empty(port->OutBuf) != DZQ_EMPTY); /* Is the queue empty? */
   while (!(inportb(port->LSR) & 0x20));           /* Is the chip empty? */
   ...
   if (port == irq_top_com_port[port->nIRQ]) {
      ...
      if (i_port != NULL) i_port->last_port = NULL;
      else {
...
inline void dos_comm_port_force_mcr_bit(comm_port *port, int bit /* 0 to 7 */, int new_state)
{
   ...
   if (new_state) outportb(port->MCR, c | m);
   else           outportb(port->MCR, c & (~m));
}
...
int dos_comm_port_interrupt_handler(comm_port *port)
{
   ...
   else outportb(0x20,0x60+port->nIRQ);
   ...
   do {
	 ...
	 if (port->next_port == NULL) int_id = -1; /* Nothing (left) to service - Thanks Mike */
	 else {
	    port = port->next_port;
	    int_id = inportb(port->IIR);
	 }
   ...
   /* Point 7 */
   outportb(c_port->IMR_8259, inportb(c_port->IMR_8259) &
			     c_port->interrupt_enable_mask);
...
#define com_irq_wrapper_(N) static int com##N##_irq_wrapper(void) {\
			       DISABLE();\
...
void dos_comm_port_test_flow_bit(comm_port *port)
{
   ...
   case XON_XOFF : // XON/XOFF
      if ((queue_nearly_full(port->InBuf) == DZQ_NOT_NEARLY_FULL) &&
	  (port->xonxoff_send != XON_SENT)) {
	 port->xon=1;
	 dos_interrupt_on(port,THREINT);
      }
...
inline void dos_comm_port_start_break(comm_port *port)
{
   outportb(port->LCR, inportb(port->LCR) | 0x40);
}
END_OF_FUNCTION(dos_comm_port_start_break);


/*--------------------- DOS COMM PORT STOP BREAK ---------------------*/
inline void dos_comm_port_stop_break(comm_port *port)
{
   outportb(port->LCR, inportb(port->LCR) & 0xbf);
}
