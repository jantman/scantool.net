...
int baud_from_baud_bits[] = {    50,     75,    110,    134,    150,
...
int comm_port_load_settings(comm_port *port,char *ini_name)
{
   ...
   do {
	 ...
	 if (!strncasecmp(line_from_file, "baud", 4)) {
	    ...
	    if      (!strcasecmp(value,"even"))  port->nParity=EVEN_PARITY;
	    else if (!strcasecmp(value,"odd"))   port->nParity=ODD_PARITY;
	    else if (!strcasecmp(value,"mark"))  port->nParity=MARK_PARITY;
	    else if (!strcasecmp(value,"space")) port->nParity=SPACE_PARITY;
	    else if (!strncasecmp(value,"no",2)) port->nParity=NO_PARITY;
	 }
	 else if (!strncasecmp(line_from_file, "control", 7)) {
	    get_value_from_line(value, line_from_file + 7);

	    if      ((!strcasecmp(value,"xon_xoff")) ||
		     (!strcasecmp(value,"xon/xoff"))) port->control_type=XON_XOFF;
	    else if ((!strcasecmp(value,"no"))       ||
		     (!strcasecmp(value,"none")))     port->control_type=NO_CONTROL;
	    else if ((!strcasecmp(value,"rts_cts"))  ||
		     (!strcasecmp(value,"rts/cts")))  port->control_type=RTS_CTS;
	 }
...
{
   port->nBaud = baud_rate;
}

void comm_port_set_data_bits(comm_port *port, data_bits num_data_bits)
{
   port->nData = num_data_bits;
}

void comm_port_set_stop_bits(comm_port *port, stop_bits num_stop_bits)
{
   port->nStop = num_stop_bits;
}

void comm_port_set_parity(comm_port *port, parity_bits parity_option)
{
   port->nParity = parity_option;
}

void comm_port_set_flow_control(comm_port *port, flow_control_type flow_option)
{
   port->control_type = flow_option;
}

void comm_port_set_lsr_handler(comm_port *port, dz_ll_handler lsr_handler)
{
   port->lsr_handler = lsr_handler;
}

void comm_port_set_msr_handler(comm_port *port, dz_ll_handler msr_handler)
{
   port->msr_handler = msr_handler;
}
...
stop_bits comm_port_get_stop_bits(comm_port *port)
{
   return port->nStop;
}
...
int comm_port_in_empty(comm_port *port)
{
   ...
   if (comm_port_funcs->io_buffer_query)
      rv = comm_port_funcs->io_buffer_query(port, DZ_IO_BUFFER_IN, DZ_IO_BUFFER_EMPTY);
...
int comm_port_out_empty(comm_port *port)
{
   ...
   /* Do the machine/OS specific thing */
   if (comm_port_funcs->io_buffer_query)
      rv = comm_port_funcs->io_buffer_query(port, DZ_IO_BUFFER_OUT, DZ_IO_BUFFER_EMPTY);
...
int comm_port_in_full(comm_port *port)
{
   ...
   /* Do the machine/OS specific thing */
   if (comm_port_funcs->io_buffer_query)
      rv = comm_port_funcs->io_buffer_query(port, DZ_IO_BUFFER_IN, DZ_IO_BUFFER_FULL);
...
int comm_port_out_full(comm_port *port)
{
   ...
   /* Do the machine/OS specific thing */
   if (comm_port_funcs->io_buffer_query)
      rv = comm_port_funcs->io_buffer_query(port, DZ_IO_BUFFER_OUT, DZ_IO_BUFFER_FULL);
